...
//        }
        ...
        for(Vertice<T> v : vertices){
            ...
            for(Arista<T> ar: v.aristas)
            {
                ...
                if(!b.p.alreadySet()){
                    ...
                    rotacion+=anguloInicial;
                    mS+=lineTag(v.p.x,v.p.y,b.p.x,b.p.y);
                }
            ...
            v.drawn = true;
            mS+= cirlceTag(v.p.x,v.p.y,cR-5, "black");
            mS+= valueTag("white", v.p.x, v.p.y+3, v.toString());
    ...
    {
        return String.format("<text fill='%s' font-family='sans-serif' font-size='10' x='%.2f' y='%.2f' text-anchor='middle'>%s</text>\n",fill, x,y,s);
        
    }
    private String lineTag(double x1, double y1, double x2, double y2)
    {
        
        return String.format("<line x1='%.2f' y1='%.2f' x2='%.2f' y2='%.2f' %s />\n",x1,y1,x2,y2, lineStyle() );
    }
    private String lineStyle()
    {
        return ("stroke='black' stroke-width='0.5'");
    }
