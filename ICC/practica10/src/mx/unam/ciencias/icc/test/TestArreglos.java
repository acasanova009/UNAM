...
public class TestArreglos {
    ...
    @Test public void testBusquedaBinaria() {
        ...
        for (int i = 0; i < total; i++)
            arreglo[i] = ini + i;
        for (int i = 0; i < total; i++)
            Assert.assertTrue(Arreglos.busquedaBinaria(arreglo, arreglo[i])
                              == i);
        Assert.assertTrue(Arreglos.busquedaBinaria(arreglo, ini - 1)
                          == -1);
        Assert.assertTrue(Arreglos.busquedaBinaria(arreglo, ini + total)
                          == -1);
    }
}
